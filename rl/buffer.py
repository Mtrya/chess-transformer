...
class ReplayBuffer:
    ...
    def push_game(self, game: Game):
        ...
        if result not in ["1-0", "0-1", "1/2-1/2"]:
            raise ValueError(
                f"Result not recognized: {result}. Either an incompleted game was passed in, or game.update_game_status() method is wrong."
            )
    ...
    def _process_trajectory(
        self, fens, reps, actions, log_probs, values, invalid_masks, final_reward
    ):
    ...
    def _compute_advantage(
        self, value_traj: torch.Tensor, final_reward: float
    ) -> torch.Tensor:
        """
    ...
    def sample(
        ...
        for start in range(0, n, self.batch_size):
            ...
            reps_b = torch.stack(
                [
                    self.repetition_counts[i].detach().clone()
                    if torch.is_tensor(self.repetition_counts[i])
            ...
            acts_b = torch.stack(
                [
                    self.actions[i].detach().clone()
                    if torch.is_tensor(self.actions[i])
            ...
            logps_b = torch.stack(
                [
                    self.log_probs[i].detach().clone()
                    if torch.is_tensor(self.log_probs[i])
            ...
            vals_b = torch.stack(
                [
                    self.values[i].detach().clone()
                    if torch.is_tensor(self.values[i])
            ...
            advs_b = torch.stack(
                [
                    self.advantages[i].detach().clone()
                    if torch.is_tensor(self.advantages[i])
            ...
            invs_b = torch.stack(
                [
                    self.invalid_masks[i]
                    if torch.is_tensor(self.invalid_masks[i])
