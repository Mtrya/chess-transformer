...
_IDX_TO_COORDS: Dict[int, Tuple[int, int]] = {
    i: (i // 8, i % 8) for i in range(64)
}  # (rank, file) 0-7
_COORDS_TO_IDX: Dict[Tuple[int, int], int] = {v: k for k, v in _IDX_TO_COORDS.items()}
_IDX_TO_ALG: Dict[int, str] = {
    i: f"{chr(ord('a') + file)}{rank + 1}" for i, (rank, file) in _IDX_TO_COORDS.items()
}
...
def generate_structurally_valid_move_map() -> Dict[str, int]:
    ...
    for from_idx in range(64):
        ...
        for to_idx in range(64):
            ...
            is_knight_move = (abs_dr == 2 and abs_df == 1) or (
                abs_dr == 1 and abs_df == 2
            )
            is_rook_move = (
                dr == 0 or df == 0
            )  # Includes King horiz/vert & pawn double push
            is_bishop_move = (
                abs_dr == abs_df
            )  # Includes King diagonal & pawn capture/push
